...
RUN --mount=type=secret,id=mytoken,required=true cp /run/secrets/mytoken /home/worker/mytoken && \
    chown worker /home/worker/mytoken

# use user instead of running as root
USER worker

# set/create the working directory that will be used as exercism workspace
WORKDIR /home/worker/workspace

#configure the CLI with the token
RUN exercism configure --token=$(cat ~/mytoken) --workspace=~/workspace
